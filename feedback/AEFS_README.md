...
你发现Agent的错 ────→ agent_learning_progress/
                       YYYY-MM-DD.md
                              │
                              ▼
                       pipeline 读取两个源，合并处理
...
```bash
# 本项目零外部依赖，clone 即可用
git clone https://github.com/zhsc-lab/my-project.git
cd my-project/agent_learning_progress/feedback
```
...
```bash
cd agent_learning_progress/feedback
python _feedback_pipeline.py --full-scan
```

### 查看状态

```bash
python _feedback_pipeline.py --status
```

### 查看某个 Agent 的预防规则

```bash
python _agent_hook_engine.py tutor      # 查看 tutor 的错题本
python _agent_hook_engine.py studyera   # 查看 studyera 的错题本
python _agent_hook_engine.py --all      # 查看所有 Agent
```
...
feedback/
├── README.md                          ← 本文件
├── _feedback_pipeline.py              ← 🔥 核心流水线（四方分析+惩罚+质检+注入）
├── _agent_hook_engine.py              ← Agent 错题本生成器（供子 Agent 自读）
├── _cross_agent_propagator.py         ← 跨 Agent 学习传播器
├── _agent_weight_injection.json       ← 预防规则数据库（自动生成）
├── _agent_penalty_tracker.json        ← 惩罚扣分追踪（自动生成）
├── _audit_log.jsonl                   ← 流水线审计日志（自动生成）
│
├── error_message/
│   ├── _ERROR_REPORT_PROTOCOL.md      ← Agent 自报错误协议
│   └── _archived/                     ← 已处理的错误报告归档
│       ├── 2026-07-22_101500_tutor_ToolCall.json
│       └── 2026-07-22_102000_studyera_Python.json
│
...
```bash
# 增量运行（只处理新增错误）
python _feedback_pipeline.py

# 全量扫描（重新分析所有日志）
python _feedback_pipeline.py --full-scan

# 试运行（只检查不修改）
python _feedback_pipeline.py --dry-run

# 查看状态
python _feedback_pipeline.py --status

# 重置某 Agent 的某错误类型分数
python _feedback_pipeline.py --reset tutor "[ToolCall]"
```
...
```bash
# 查看特定 Agent 的错题本
python _agent_hook_engine.py tutor

# 查看所有 Agent
python _agent_hook_engine.py --all

# 仅供 Hook 系统调用（静默检查）
python _agent_hook_engine.py --check
python _agent_hook_engine.py --notify
```
...
```bash
# 手动触发传播
python _cross_agent_propagator.py --upgrade tutor "ToolCall" 1.5

# 审查当前跨源规则
python _cross_agent_propagator.py --review
```
...
```
每日流程：
  1. python _feedback_pipeline.py              # 收集今天的错误
  2. python _agent_hook_engine.py --all        # 查看各 Agent 新增规则
  
Agent 启动时：
  1. python _agent_hook_engine.py tutor         # 自读错题本
  2. 逐条确认预防规则后开始工作
  
常规运维：
  1. python _feedback_pipeline.py --status      # 查看各 Agent 扣分情况
  2. python _feedback_pipeline.py --full-scan   # 周末全量扫描
```
...
### 10.1 常量配置（`_feedback_pipeline.py` 顶部）
...
1. 运行以下命令查看你的历史犯规记录：
   ```bash
   python agent_learning_progress/feedback/_agent_hook_engine.py tutor
   ```
...
{
  "PostToolUse": {
    "command": "python .claude/scripts/_injection_hook.py",
    "timeout": 15000,
    "async": true
  }
}
...
Agent 执行出错时应立即写 JSON 到 `error_message/`：
...
- **TEAL V2.0** `_terminal_monitor.py`：终端错误自动检测层升级
  - 新增 `update_report()` 函数：修复成功后补充实际解决方案到已上报的错误报告
  - 新增 `--update` CLI 模式：`python _terminal_monitor.py --update 文件名.json "解决方案"`
  - 新增 `source` 字段标记 `+fixed`：区分已修复和未修复的报告
  - 新增 `_teal_meta.updated_at` / `update_count`：记录报告修改历史和版本
- **`teal.cmd` 增强**：支持 `update` 子命令，如 `teal update 文件.json "解决方案"`
- **Agent 自动错误检测协议 V2.0**：从"手动自报"升级为"自动检测上报"
  - 每次工具调用后自动检查 exit_code/stderr/工具错误
  - 检测即上报：错误出现后 **先 teal 上报再修复**
  - 修复后更新：`teal update` 写入实际解决方案
  - 闭环完成：上报 → 修复 → update → AEFS 处理 → 跨 session 继承
- **严重度判定体系完整文档化**：
  - 三级划分：1(小问题 ) / 2(中等) / 3(严重)
  - 判定优先级：stderr 模式匹配 → exit_code 映射 → 描述关键词 → 默认 2
  - exit_code 映射表：124(超时)=2、130(Ctrl+C)=1、137(OOM)=3、139(Segfault)=3 等
  - 描述关键词表：每个严重度对应触发词列表

#### 🔧 优化

- Agent 不再需要等待用户发现错误——自动检测，立即上报
- 修复后解决方案自动写回报告，AEFS 提取时获得完整 solution 字段
- 简化使用：`teal`（无描述兜底）/ `teal pip 超时`（一句话上报）/ `teal update`（修复后补充）

### [1.0.0] — 2026-07-21

#### 🚀 新增

- **核心流水线** `_feedback_pipeline.py`：五步完整闭环（扫描→四方分析→惩罚→质检→注入）
- **四方分析模块**：Qwen 完整性检查 + GLM 根因质检 + Kimi 标签审计 + DeepSeek 仲裁合成
- **惩罚机制**：3 分制递进扣分 + 权重阶梯(1.0×/1.5×/2.0×/3.0×)
- **质量门禁**：≥9.6 阈值 + 最多 3 轮迭代修订
- **知识注入**：自动生成预防规则并写入 `_agent_weight_injection.json`
- **跨 Agent 学习**：`_cross_agent_propagator.py` — 通用标签 50% 权重传播
- **Hook 引擎**：`_agent_hook_engine.py` — 子 Agent 自读错题本 + PostToolUse 通知
- **双源错误收集**：自动上报 JSON + 手动记录 Markdown 双通道
- **审计日志**：每次运行的完整流水线记录写入 `_audit_log.jsonl`
